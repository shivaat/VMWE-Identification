...
class Corpus:
    ...
    def readCuptFile(cuptFile):
        ...
        with open(cuptFile) as corpusFile:
            ...
            for line in lines:
                ...
                if line.startswith('# sent_id:'):
                    ...
                    sentenceText = line.split('# text =')[1].strip()    # SHIVA ADDED
                    continue
                ...
                if not line.startswith('#'):
                    ...
                    if len(lineParts) != 11 or '-' in lineParts[0]:     # SHIVA CHANGED it to 11
                        continue
                    morpho = ''
                    if lineParts[5] != '_':
                        morpho = lineParts[5].split('|')
                    if lineParts[6] != '_' and lineParts[6] != '-':     # SHIVA added the and part
                        token = Token(lineParts[0], lineParts[1], lemma=lineParts[2],   ## SHIVA I removed .lower() from lineParts[1]
                        ...
                        # SHIVA: It does never come to this else. Once it got here and it gave errors.
        ...
        return sentences, mweNum        # SHIVA ADDED
...
class Token:
    ...
    def __init__(self, position, txt, lemma='', posTag='', abstractPosTag='', morphologicalInfo=[], dependencyParent=-1,
                 dependencyLabel=''):
        try:    # SHIVA: added only for one case of line number 24.1 in he English data
            self.position = int(position)
        except ValueError:  # SHIVA: added only for one case of line number 24.1 in he English data
